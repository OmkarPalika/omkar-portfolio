...
function MainProfile() {
  return (
    <div className="bg-gradient-to-br from-[var(--color-brand-green)] via-green-800 to-green-950 rounded-3xl overflow-hidden shadow-2xl h-full relative group">
      {/* Large Profile Image with optimized loading strategy */}
      <div className="h-full relative">
        <div className="relative w-full h-full min-h-[650px] md:min-h-[450px] lg:min-h-[550px]">
          <Image
        ...
        {/* Name Card with improved positioning and animations */}
        <div className="absolute inset-0 flex flex-col justify-end items-center pb-10 md:pb-14 lg:pb-20 transform transition-transform duration-500 group-hover:translate-y-6 md:translate-y-8 lg:translate-y-12 lg:translate-x-[-60px] xl:translate-x-[-80px]">
          <div className="w-11/12 md:w-4/5 lg:w-4/6 lg:mb-2 px-6">
            <div className="bg-white/10 backdrop-blur-md rounded-3xl border border-white/30 shadow-lg px-5 py-4 md:px-6 md:py-5 text-center transition-all duration-300 group-hover:bg-white/15">
              ...
              {/* Navigation Buttons with improved interactions */}
              <div className="flex flex-col md:flex-row gap-3 lg:gap-5 w-full justify-center items-center">
                <Link 
                  href="#projects" 
                  className="w-fit md:w-1/2 bg-[var(--color-brand-green)] text-white px-5 md:px-6 lg:px-8 py-2 md:py-2.5 lg:py-3 rounded-full text-sm md:text-base hover:bg-green-700 transition-all duration-300 shadow-lg hover:shadow-xl flex items-center justify-center gap-2 group focus:outline-none focus:ring-2 focus:ring-white/50"
                  scroll={true}
                >
                  <span>Projects</span>
                ...
                <Link 
                  href="#services" 
                  className="w-fit md:w-1/2 bg-black text-white px-5 md:px-6 lg:px-8 py-2 md:py-2.5 lg:py-3 rounded-full text-sm md:text-base hover:bg-[var(--color-brand-green)] transition-all duration-300 shadow-lg hover:shadow-xl flex items-center justify-center gap-2 group focus:outline-none focus:ring-2 focus:ring-white/50"
                  scroll={true}
                >
                  <span>Services</span>
                  <ChevronRightIcon className="w-4 h-4 transform transition-transform duration-300 group-hover:translate-x-1" />
