    ...
    <div className="flex flex-col justify-between gap-4">
      <div>
        <h2 className="text-xl font-semibold flex items-center">
          <svg className="w-5 h-5 mr-2 text-[var(--color-brand-green)] group-hover:scale-110 transition-transform duration-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
      ...
      <div className="grid gap-4 grid-cols-5 mb-3 mt-4 animate-fade-in">
        {skills.map((skill, i) => (
          <div className="flex justify-center" key={`skill-${i}`} title={skill.label}>
            <div className="w-10 h-10 flex items-center justify-center rounded-lg shadow-md shadow-green-600/20 transform hover:rotate-6 hover:scale-110 transition-all duration-300" aria-label={skill.label}>
              {skill.type === "icon" ? 
                <i className={`${skill.content} ${skill.textColor || "text-white"} text-2xl`}/> :
